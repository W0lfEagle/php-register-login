...
class FGMembersite
{
    ...
    {
        $this->sitename = 'wilfengel.com';
        $this->rand_key = 'asljicb7e9q7asdk92';
    }
    ...
    function CheckLoginInDB($email,$password)
    {
        ...
        $username = $this->SanitizeForSQL($email);
        $pwdmd5 = md5($password);
        $qry = "Select name, email from $this->tablename where email='$email' and password='$pwdmd5' and confirmcode='y'";
        ...
        function CollectRegistrationSubmission(&$formvars)
    {
        $formvars['name'] = $this->Sanitize($_POST['name']);
        $formvars['email'] = $this->Sanitize($_POST['email']);
        $formvars['username'] = $this->Sanitize($_POST['name']);
        $formvars['password'] = $this->Sanitize($_POST['password']);
    }
